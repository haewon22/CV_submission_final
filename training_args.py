...
class WarmupCosineLR(_LRScheduler):
    def __init__(self, optimizer, T_max: int, cur_iter: int = 0, warmup_factor: float = 1.0 / 3, warmup_iters: int = 500, eta_min: float = 0.0):
    ...
    def get_lr(self):
        ...
        progress = (self.last_epoch - self.warmup_iters) / float(max(1, self.T_max - self.warmup_iters))
...
class WarmupPolyLR(_LRScheduler):
    def __init__(self, optimizer, T_max: int, cur_epoch: int = 0, warmup_epochs: int = 5, warmup_factor: float = 1.0 / 10, power: float = 0.9, eta_min: float = 0.0):
        self.T_max         = T_max
    ...
    def get_lr(self):
        ...
        return [self.eta_min + (base_lr - self.eta_min) * poly for base_lr in self.base_lrs]
...
class UniformCBCE_LovaszProb(nn.Module):
    def __init__(self, num_classes: int, bg_factor: float = 0.05, coef_ce: float = 0.6, coef_iou: float = 0.4, eps: float = 1e-8):
        super().__init__()
        self.eps = eps
    ...
    def forward(self, probs, target):
        ...
        if target.ndim == 4:
            ...
            target = target.long()
        loss_ce = self.ce2d_deterministic(logits=torch.log(probs.clamp(min=self.eps)), target=target, weight=self.ce_weight.to(probs.device))
